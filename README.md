...
3. 点击联系人列表旁的 **＋**，输入好友邮箱。对方也需要使用 Slip；收到好友请求后点击接受，即可自动完成公钥交换。
...
好友申请先保存到本地队列，再发送到对方邮箱。对方不在线时，邮件由其邮箱保存；启动 Slip 后会自动补收，并显示“收到好友请求”。陌生请求需要点击接受，已主动发出的申请收到回复后自动完成交换，不会互相循环回信。发送失败会退避重试，重启后保留待发送请求；发送方需运行 Slip 才能继续发送本地队列。

连接支持 IMAP IDLE 的邮箱后，新邮件通知会触发同步，随后通知界面刷新；不支持 IDLE 时每 15 秒检查一次。网络断开会自动重连，回到窗口时也会补收。邮件服务商的投递、过滤和限流仍可能造成延迟，因此不保证即时送达，也不显示推测的好友在线状态。“等待对方确认”仅表示申请已交给发送服务器，不代表对方已收到或已读。
