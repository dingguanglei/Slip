...
mod tests {
    use super::{
        Identity, decrypt_secret, encrypt_secret, fingerprint_of_b64, grouped_fingerprint,
        random_id,
    };

    #[test]
    fn identity_encryption_round_trips_and_rejects_wrong_passphrase() {
        let secret = [7u8; super::KEY_LEN];
        let enc = encrypt_secret(&secret, "correct horse").unwrap();
        // Stored ciphertext is not the plaintext key (base64 of 32 zeros/sevens).
        assert!(!enc.ciphertext.is_empty());
        assert_ne!(enc.salt, enc.nonce);
        assert_eq!(decrypt_secret(&enc, "correct horse").unwrap(), secret);
        assert!(decrypt_secret(&enc, "wrong").is_err());
    }

    #[test]
    fn grouped_fingerprint_reads_in_four_blocks() {
        assert_eq!(
            grouped_fingerprint("1a2b3c4d5e6f7a8b"),
            "1a2b 3c4d 5e6f 7a8b"
        );
    }
    ...
    fn seal_open_round_trip() {
        ...
        let sealed = alice.seal(&bob.public_key_b64(), b"hello bob").unwrap();
        assert_ne!(&sealed[super::NONCE_LEN..], b"hello bob".as_slice());
        let opened = bob.open(&alice.public_key_b64(), &sealed).unwrap();
    ...
    fn open_rejects_wrong_sender_key() {
        ...
        let sealed = alice.seal(&bob.public_key_b64(), b"secret").unwrap();
        assert!(bob.open(&mallory.public_key_b64(), &sealed).is_err());
    }
    ...
    fn open_rejects_truncated_payload() {
        let alice = Identity::generate();
        let bob = Identity::generate();
        assert!(bob.open(&alice.public_key_b64(), &[0u8; 10]).is_err());
    }

    #[test]
    fn fingerprints_are_stable_hex16() {
        let alice = Identity::generate();
        let fp = fingerprint_of_b64(&alice.public_key_b64()).unwrap();
        assert_eq!(fp.len(), 16);
        assert_eq!(fp, alice.fingerprint());
        assert!(fingerprint_of_b64("not-base64!!").is_err());
    }

    #[test]
    fn random_ids_are_unique_hex32() {
        let a = random_id();
        let b = random_id();
        assert_eq!(a.len(), 32);
        assert_ne!(a, b);
