...
mod tests {
    ...
    #[test]
    fn level_parsing_covers_words_and_switches() {
        ...
        // Unknown non-empty values default to info rather than silently off.
        assert_eq!(parse_level("chatty"), Some(Level::Info));
    }

    #[test]
    fn values_with_spaces_are_quoted() {
        assert_eq!(quote_value("Slip"), "Slip");
    ...
    #[test]
    fn registered_secrets_are_masked_everywhere() {
        register_secret("hunter2pass");
        let line = scrub("imap login user=alice pass=hunter2pass ok".to_string());
        assert!(!line.contains("hunter2pass"));
        assert!(line.contains("***"));
        // A body that happens to embed the secret is masked too.
        let body = scrub("send preview=\"here is hunter2pass again\"".to_string());
        assert!(!body.contains("hunter2pass"));
    }

    #[test]
    fn short_secrets_are_not_registered() {
        register_secret("ab");
        let line = scrub("value=ab".to_string());
        // "ab" is too short to register, so it is left intact.
